...
<div id="content">
    ...
    <p>Here are the list of patients who you have visited you today, i.e. <?php echo date('dS F, Y')?>. The diagnosis is also included for your reference:</p>
	...
	<table>
		...
		<tbody>
            <?php
                $patients_seen = get_seenPatients_today($_id);
                $alt_count = 0;
                if(sqlsrv_has_rows($patients_seen) == FALSE)
                {
                    echo "Sorry you have not seen any patient today.</br>";
                }
                else
                {
                    while( $row = sqlsrv_fetch_array( $patients_seen) )
                    {
                        ...
                        if($alt_count%2==0)
                        {
                            echo "<tr> <td>". $row['name']."</td> <td>".$row['gender']."</td> <td>". date_format($row['datetime'], 'h:i A')."</td> <td>". $row['diagnosis']."</td> </tr>";
                        }
                        else
                        {
                            echo "<tr class=\"alt\"> <td>". $row['name']."</td> <td>".$row['gender']."</td> <td>". date_format($row['datetime'], 'h:i A')."</td> <td>". $row['diagnosis']."</td> </tr>";
                        }
                        $alt_count++;
                    }
                }
